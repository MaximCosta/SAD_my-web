...
echo "== Checking the existing VM for my-web..."
echo "> Checking if the image file already exists..."
if [ ! -f "images/sad-my_web-web.img" ]; then
...
echo "> Creating VM for my-web using virt-install at $1 (specs: UEFI, 2GB of RAM, 32GB of hard drive, network bridge, graphics via VNC on 127.0.0.1:5901)..."
virt-install -n"B-SAD-200_my-web_francois.lefevre" --memory=2048 --disk path=images/sad-my_web-web.img,size=32 --network bridge=virbr0 --graphics vnc,listen=127.0.0.1,port=5901 --noautoconsole --boot uefi
